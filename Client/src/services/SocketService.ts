...
export class SocketService {
    ...
    constructor(token: string, authService: AuthService) {
        ...
        // Fix: Use type assertion for custom events
        window.addEventListener('send-message-request', this.handleSendMessageRequest as EventListener);

        // Listen for beforeunload to set status to offline
        window.addEventListener('beforeunload', this.handleBeforeUnload.bind(this));

        // Listen for logout events - also needs type assertion
    ...
    private startConnectionChecker(): void {
        this.stopConnectionChecker(); // Clear any existing interval

        // Check connection status every 10 seconds
        this.connectionCheckInterval = window.setInterval(() => {
            if (this.socket) {
                if (this.socket.readyState === WebSocket.CLOSED || this.socket.readyState === WebSocket.CLOSING) {
                    console.warn("[SocketService] Connection check failed, socket is closed");
                    this.handleReconnect();
                }
            } else if (this.userId && this.username && !this.isConnecting) {
                console.warn("[SocketService] Connection check failed, no socket exists");
  ...
  private handleDirectMessageReceived(data: any): void {
        ...
        // Check if we've already processed this message (less aggressive deduplication)
        const messageKey = `${from}_${text.substring(0, 20)}`;
        const lastProcessed = this.processedMessages.get(messageKey);

        if (lastProcessed !== undefined) {
            const now = Date.now();
            // Only consider it a duplicate if processed in the last 5 seconds
            if (now - lastProcessed < 5000) {
                console.log('[SocketService] Duplicate message detected, ignoring:', messageKey);
                return;
            }
        }
        ...
        // Limit the size of the processed messages map
    ...
    public sendDirectMessage(to: string, text: string): void {
        ...
        try {
            ...
            // Create a unique message ID for this sent message
            const messageId = `sent_${this.username}_${targetUsername}_${Date.now()}`;
            ...
            // Also manually trigger a received event if we're sending to ourselves
            // (for testing/debugging)
            if (targetUsername === this.username) {
                console.log("[SocketService] Message sent to self, triggering received event");
                window.dispatchEvent(new CustomEvent('direct-message-received', {
                    detail: {
                        from: this.username,
                        text: messageText,
