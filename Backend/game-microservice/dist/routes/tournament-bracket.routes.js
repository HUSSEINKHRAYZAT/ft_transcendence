...
exports.tournamentBracketRoutes = tournamentBracketRoutes;
const tournament_bracket_service_1 = require("../services/tournament-bracket.service");
async function tournamentBracketRoutes(app) {
    const bracketService = (0, tournament_bracket_service_1.tournamentBracketService)(app);
    /**
     * Create a new tournament
     */
    app.post('/api/tournaments/create', async (request, reply) => {
        try {
            const { name, size, createdBy, createdByName } = request.body;
            // Validate size
    ...
    app.post('/api/tournaments/join', async (request, reply) => {
        try {
            const { code, userId, username } = request.body;
            // Get tournament
    ...
    app.post('/api/tournaments/match/:id/complete', async (request, reply) => {
        try {
            const { id } = request.params;
            const { winnerId, scorePlayer1, scorePlayer2 } = request.body;
            const matchId = parseInt(id);
...
//# sourceMappingURL=tournament-bracket.routes.js.map