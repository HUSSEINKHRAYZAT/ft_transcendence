...
function generateBracket(size, players) {
    const matches = [];
...
function tournamentsService(app) {
    const db = app.db;
    // Create tournament
    function createTournament(input) {
        ...
        VALUES (?, ?, ?, 0, 'waiting', ?, ?, ?, 0, ?, ?)
      `).run(tournamentId, input.name, input.size, input.createdBy, input.isPublic ? 1 : 0, input.allowSpectators ? 1 : 0, now, now);
    ...
    function getAllTournaments() {
        ...
        LIMIT 100
      `).all();
            return rows.map((row) => enrichTournament(row));
        }
        catch (error) {
    ...
    function getTournamentById(id) {
        try {
            ...
            return row ? enrichTournament(row) : null;
        }
        catch (error) {
    ...
    `).get(input.tournamentId, input.playerId);
      ...
      `).run(new Date().toISOString(), input.tournamentId);
            ...
            if (!updated)
                throw new Error('Failed to get updated tournament');
            return updated;
        }
        catch (error) {
    ...
    function startTournament(input) {
        ...
        try {
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?)
        `).run(dbId, match.id, match.round, match.matchIndex, match.player1?.id || null, match.player2?.id || null, matchStatus);
            });
    ...
    // Update match result
    function updateMatch(tournamentId, input) {
    ...
    function enrichTournament(row) {
      ...
      `).all(row.id);
            ...
            const matchesArray = matches.map((m) => {
                ...
                const statusValues = ['pending', 'active', 'completed'];
