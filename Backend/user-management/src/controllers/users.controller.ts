...
const plugin: FastifyPluginAsync = async (app: FastifyInstance) => {
  ...
  app.post('/lookup', {
    ...
    let user;
    if (username) user = svc.getUserByUsername(username);
  ...
  app.post('/oauth-upsert', {
    schema: {
      ...
      response: { 200: User },
      summary: 'Create or update a user coming from OAuth (no mailer, verified=1)',
    },
  }, async (req) => {
    const id = await svc.upsertOAuthUser(req.body as any);
