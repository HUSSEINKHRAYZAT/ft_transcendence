export const SCHEMA_SQL = `
BEGIN;
PRAGMA foreign_keys = ON;
CREATE TABLE IF NOT EXISTS users (
  id INTEGER PRIMARY KEY,
  firstName TEXT NOT NULL,
  lastName TEXT NOT NULL,
  username TEXT NOT NULL UNIQUE,
  email TEXT NOT NULL UNIQUE,
  hashedPassword TEXT NOT NULL,
  isVerified INTEGER NOT NULL DEFAULT 0,
  isLoggedIn INTEGER NOT NULL DEFAULT 0,
  twoFactorEnabled INTEGER NOT NULL DEFAULT 0,
  profilePath TEXT,
  status TEXT NOT NULL,
  createdAt TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),
  updatedAt TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now'))
);
CREATE TABLE IF NOT EXISTS statistics (
  id INTEGER PRIMARY KEY,
  userId INTEGER NOT NULL,
  winCount INTEGER NOT NULL DEFAULT 0 CHECK (winCount >= 0),
  lossCount INTEGER NOT NULL DEFAULT 0 CHECK (lossCount >= 0),
  tournamentWinCount INTEGER NOT NULL DEFAULT 0 CHECK (tournamentWinCount >= 0),
  tournamentCount INTEGER NOT NULL DEFAULT 0 CHECK (tournamentCount >= 0),
  createdAt TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),
  updatedAt TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),
  UNIQUE (userId),
  FOREIGN KEY (userId) REFERENCES users(id) ON DELETE CASCADE
);
CREATE INDEX IF NOT EXISTS idx_statistics_userId ON statistics(userId);
CREATE TABLE IF NOT EXISTS user_relation_type (
  id INTEGER PRIMARY KEY,
  type TEXT NOT NULL UNIQUE,
  createdAt TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now'))
);
CREATE TABLE IF NOT EXISTS user_relation (
  id INTEGER PRIMARY KEY,
  userOneId INTEGER NOT NULL,
  userTwoId INTEGER NOT NULL,
  typeId INTEGER NOT NULL,
  createdAt TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),
  updatedAt TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),
  CHECK (userOneId <> userTwoId),
  UNIQUE (typeId, userOneId, userTwoId),
  FOREIGN KEY (userOneId) REFERENCES users(id) ON DELETE CASCADE,
  FOREIGN KEY (userTwoId) REFERENCES users(id) ON DELETE CASCADE,
  FOREIGN KEY (typeId) REFERENCES user_relation_type(id) ON DELETE CASCADE
);
CREATE INDEX IF NOT EXISTS idx_ur_userTwoId ON user_relation(userTwoId);
CREATE INDEX IF NOT EXISTS idx_ur_userOneId ON user_relation(userOneId);
DROP VIEW IF EXISTS statistics_view;
CREATE VIEW statistics_view AS SELECT s.*, (s.winCount + s.lossCount) AS totalGames FROM statistics s;
CREATE TABLE IF NOT EXISTS languages (
  code TEXT PRIMARY KEY,
  name TEXT NOT NULL
);
INSERT OR IGNORE INTO languages (code, name) VALUES ('en', 'English');
INSERT OR IGNORE INTO languages (code, name) VALUES ('fr',  'Français');
INSERT OR IGNORE INTO languages (code, name) VALUES ('de',  'Deutsch');
CREATE TABLE IF NOT EXISTS settings (
  id INTEGER PRIMARY KEY,
  username TEXT NOT NULL UNIQUE,          -- string from frontend
  languageCode TEXT NOT NULL DEFAULT 'eng', -- FK to languages(code)
  accentColor TEXT NOT NULL DEFAULT 'lime', -- string from frontend
  backgroundTheme TEXT NOT NULL DEFAULT 'dark', -- string from frontend
  createdAt TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),
  updatedAt TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),
  FOREIGN KEY (languageCode) REFERENCES languages(code) ON UPDATE CASCADE
);
CREATE INDEX IF NOT EXISTS idx_settings_username ON settings(username);
CREATE INDEX IF NOT EXISTS idx_settings_languageCode ON settings(languageCode);
COMMIT;`;