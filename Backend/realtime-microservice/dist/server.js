...
function activateNextMatchIfAvailable(tournament) {
    ...
    if (allCurrentRoundComplete && tournament.currentRound < totalRoundsForSize(tournament.size)) {
        ...
        const nextRoundMatches = tournament.matches.filter(m => m.round === tournament.currentRound && m.player1 && m.player2 && m.status === 'pending');
...
function advanceWinnerToNextRound(tournament, completedMatch, winner) {
    ...
    const siblingMatch = tournament.matches.find(m => m.round === completedMatch.round && m.matchIndex === siblingMatchIndex);
    ...
    const pendingPlayer1 = nextMatch.pendingPlayer1;
    const pendingPlayer2 = nextMatch.pendingPlayer2;
    // ENHANCED LOGGING
    ...
    if (pendingPlayer1 || pendingPlayer2) {
        ...
        if (isPlayer1Slot) {
            nextMatch.player1 = winner;
            if (siblingWinner)
                nextMatch.player2 = siblingWinner;
        }
        else {
            if (siblingWinner)
                nextMatch.player1 = siblingWinner;
            nextMatch.player2 = winner;
        }
        // Both players assigned - activate the match
        if (nextMatch.player1 && nextMatch.player2 && nextMatch.status === 'pending') {
        ...
        if (isPlayer1Slot) {
            nextMatch.pendingPlayer1 = winner;
        }
        else {
            nextMatch.pendingPlayer2 = winner;
        }
    }
