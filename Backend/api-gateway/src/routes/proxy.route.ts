...
const routes: FastifyPluginAsync = async (app) => {
  ...
  const isPublic = (req: any) => {
    ...
    if (url.startsWith('/authWithGoogle')) return true;
    return false;
  };
  ...
  app.all('/settings', async (req, reply) => {
    const target = `${app.config.USER_SERVICE_URL}${req.raw.url}`;
    return reply.from(target, {
      rewriteRequestHeaders: () => buildUpstreamHeaders(req),
    });
  ...
  app.all('/relation', async (req, reply) => {
    const target = `${app.config.USER_SERVICE_URL}${req.raw.url}`;
    return reply.from(target, {
      rewriteRequestHeaders: () => buildUpstreamHeaders(req),
    });
  ...
  app.all('/relationType', async (req, reply) => {
    const target = `${app.config.USER_SERVICE_URL}${req.raw.url}`;
    return reply.from(target, {
      rewriteRequestHeaders: () => buildUpstreamHeaders(req),
    });
  ...
  app.all('/tournaments', async (req, reply) => {
    const target = `${app.config.GAME_SERVICE_URL}${req.raw.url}`;
    return reply.from(target, {
      rewriteRequestHeaders: () => buildUpstreamHeaders(req),
    });
  });
  ...
  app.all('/game_players', async (req, reply) => {
    const target = `${app.config.GAME_SERVICE_URL}${req.raw.url}`;
    return reply.from(target, {
      rewriteRequestHeaders: () => buildUpstreamHeaders(req),
    });
  ...
  app.all('/authWithGoogle/*', async (req, reply) => {
    const target = `${app.config.GOOGLE_OAUTH_URL}${req.raw.url}`; // forward path intact
    return reply.from(target, { rewriteRequestHeaders: () => ({}) }); // no special headers needed
  });
};
