...
// Detect environment using Vite's import.meta.env (replaced at build time)
// In production build: import.meta.env.DEV = false
// In dev mode (npm run dev): import.meta.env.DEV = true
const IS_DEV_MODE = import.meta.env.DEV;

// Use relative URLs when running production build (served by nginx)
// Use direct URLs when running Vite dev server
export const API_BASE_URL = IS_DEV_MODE
  ? `https://${window.location.hostname}:8080`  // Direct access for Vite dev server
  : '/api';  // Proxied by nginx in production build

export const WS_URL = IS_DEV_MODE
  ? `wss://${window.location.hostname}:3005/ws`  // Direct access for Vite dev server
  : `${window.location.protocol === 'https:' ? 'wss:' : 'ws:'}//${window.location.host}/ws`;  // Proxied by nginx in production
