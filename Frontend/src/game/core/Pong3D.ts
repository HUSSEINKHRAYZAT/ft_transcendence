  ...
  private initSocketIO() {
    ...
    socketManager.on("player_input", (data) => {
      if (this.isHost && data.playerId !== socketManager.id) {
        // Convert Web socket input format to our internal format
    ...
    socketManager.on("game_started", (data) => {
      ...
      if (this.isGuest && !this.gameState.matchReady) {
        this.hideWaitingOverlay();
        // Show countdown for Web socket joiner too
        const countdown = new GameCountdown({
          onComplete: () => {
            this.gameState.matchReady = true;
            this.resetBall(Math.random() < 0.5 ? 1 : -1);
    ...
    try {
      ...
      if (this.isGuest) {
        const sendInputs = () => {
          ...
          // Check if view is rotated and invert controls accordingly
          ...
          if (this.config.playerCount === 2) {
            ...
            // Invert if view is rotated (joiner perspective)
            ...
            // Invert if view is rotated
            neg = isViewRotated ? !!this.keys["arrowdown"] : !!this.keys["arrowup"];
            pos = isViewRotated ? !!this.keys["arrowup"] : !!this.keys["arrowdown"];
          } else if (idx === 2) {
            // Bottom(+Z) → move X (neg=left, pos=right)
            // For bottom paddle, left/right also need rotation adjustment
            neg = isViewRotated ? !!this.keys["arrowright"] : !!this.keys["arrowleft"];
            pos = isViewRotated ? !!this.keys["arrowleft"] : !!this.keys["arrowright"];
          } else if (idx === 3) {
            // Top(-Z) → move X mirrored (neg=right, pos=left)
            // For top paddle, controls are already mirrored, rotation reverses this
            neg = isViewRotated ? !!this.keys["arrowleft"] : !!this.keys["arrowright"];
            pos = isViewRotated ? !!this.keys["arrowright"] : !!this.keys["arrowleft"];
          }
