...
import { addBasicJumbotron } from '../../main';
/**
...
export class ModalService {
 ...
 /**
   * Show play game modal with game mode selection
   */
showPlayGameModal(): void {
  console.log('🎮 Opening play game modal...');
  ...
  this.activeModal = 'play-game';
  this.createBackdrop();

  const modalContent = createElement('div', {
    className: 'bg-gray-800 rounded-lg shadow-2xl max-w-lg w-full mx-4 p-6 transform transition-all duration-300 scale-95 opacity-0',
    innerHTML: `
      <div class="flex justify-between items-center mb-6">
        <h2 id="modal-title" class="text-2xl font-bold text-lime-500">🎮 Single Player</h2>
  ...
  this.setupGameCarousel();
  this.animateIn();
}

private setupGameCarousel(): void {
  let currentMode = 0; // 0: Single, 1: Local, 2: Online, 3: Tournament
  const modes = ['single', 'local', 'online', 'tournament'];
  const titles = ['🎮 Single Player', '🏠 Local Play', '🌐 Online Play', '🏆 Tournament'];
  ...
  if (form) {
    form.addEventListener('submit', (e) => this.handleCarouselGameSubmit(e, modes[currentMode]));
  }
  ...
  const updateMode = () => {
    ...
    // Update title
    modalTitle.textContent = titles[currentMode];
    ...
    if (prevBtn) (prevBtn as HTMLButtonElement).disabled = currentMode === 0;
    if (nextBtn) (nextBtn as HTMLButtonElement).disabled = currentMode === modes.length - 1;

    // Update content with fade effect
    modeContent.style.opacity = '0';
    setTimeout(() => {
      modeContent.innerHTML = this.getModeContent(modes[currentMode]);
      modeContent.style.opacity = '1';
      this.setupModeSpecificHandlers(modes[currentMode]);
    }, 150);
  };
...
private handleCarouselGameSubmit(event: Event, mode: string): void {
  ...
  switch (mode) {
    case 'single':
      ...
      gameConfig = {
        players: 1,
        aiPlayers: parseInt(aiPlayers) || 1,
        difficulty: 'settings', // Will be handled by settings
        type: 'single',
        online: false
      };
      break;
    ...
    case 'local':
      ...
      gameConfig = {
        players: parseInt(localPlayers) || 2,
        aiPlayers: 0,
        difficulty: 0,
        type: 'local',
        online: false
    ...
    case 'online':
      ...
      gameConfig = {
        players: invitedCount + 1, // +1 for current player
        aiPlayers: 0,
        difficulty: 0,
        type: 'remote',
        online: true
    ...
    case 'tournament':
      ...
      gameConfig = {
        players: parseInt(tournamentSize) || 4,
        aiPlayers: 0,
        difficulty: 0,
        type: 'tournament',
        online: false
  ...
  // TODO: Handle game start with gameConfig
}


/**
 * Handle game form submission and return the required data structure
 */
private handleGameFormSubmit(event: Event): void {
  event.preventDefault();

  const form = event.target as HTMLFormElement;
  const formData = new FormData(form);
  const errorDiv = findElement('#game-error');
  ...
  const gameStatus = formData.get('gameStatus') as string;
  const numberOfPlayers = formData.get('numberOfPlayers') as string;

  // Validate required fields
  if (!gameType) {
    this.showGameFormError('Please select a game type');
    return;
  }

  if (gameType === 'multi' && !gameStatus) {
    this.showGameFormError('Please select connection type for multiplayer');
    return;
  }

  if (gameType === 'multi' && !numberOfPlayers) {
    this.showGameFormError('Please select number of players for multiplayer');
    return;
  }

  // Build the required data structure
  const gameConfig = {
    type: gameType, // 'single' or 'multi'
    status: gameType === 'single' ? null : gameStatus, // 'local' or 'online' (null for single)
    numberOfPlayers: gameType === 'single' ? 1 : parseInt(numberOfPlayers), // 1, 2, or 4
    difficulty: 0 // Always 0 - will be controlled from settings elsewhere
  };

  console.log('🎮 Game Configuration Result:');
  console.log('Type:', gameConfig.type);
  console.log('Status:', gameConfig.status);
  console.log('Number of Players:', gameConfig.numberOfPlayers);
  console.log('Difficulty:', gameConfig.difficulty);
  console.log('Full Object:', gameConfig);

  // Close modal
  this.closeModal();

  // Show success with configuration details
  let configText = `Type: ${gameConfig.type}`;
  if (gameConfig.status) {
    configText += `, Status: ${gameConfig.status}`;
  }
  configText += `, Players: ${gameConfig.numberOfPlayers}`;

  this.showToast('success', 'Game Configuration Ready!', configText);

  // TODO: Here you would normally call your game handler with gameConfig
  // For now, we just return/log the configuration

  // Example of what you might do later:
  // this.startGame(gameConfig);
  // or
  // window.dispatchEvent(new CustomEvent('game-start', { detail: gameConfig }));
}

/**
 * Show error in the game form
 */
private showGameFormError(message: string): void {
  const errorDiv = findElement('#game-error');
  if (errorDiv) {
  ...
  private async handleLogin(event: Event): Promise<void>
  {
    ...
    console.log("hello from hererererererererr");
  ...
  // In ModalService.ts - Add this simplified version for testing
/**
 * Show a simple play game modal for testing
 */
showPlayGameModalTest(): void {
  console.log('🎮 Opening TEST play game modal...');
  ...
  this.activeModal = 'play-game-test';
  this.createBackdrop();
  ...
  const modalContent = createElement('div', {
    ...
    innerHTML: `
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-lime-500">🎮 Game Mode Selection</h2>
        <button id="modal-close" class="text-gray-400 hover:text-white text-2xl transition-colors duration-300">&times;</button>
      </div>

      <div class="space-y-4 mb-6">
        <button id="single-player-btn" class="w-full bg-lime-500 hover:bg-lime-600 text-white font-bold py-3 px-4 rounded transition-all duration-300">
          🤖 Single Player
        </button>

        <button id="multiplayer-btn" class="w-full bg-blue-500 hover:bg-blue-600 text-white font-bold py-3 px-4 rounded transition-all duration-300">
          👥 Multiplayer
        </button>

        <button id="ai-tournament-btn" class="w-full bg-purple-500 hover:bg-purple-600 text-white font-bold py-3 px-4 rounded transition-all duration-300">
          🏆 Tournament
        </button>
      </div>

      <button id="cancel-game" class="w-full bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded transition-all duration-300">
        Cancel
      </button>
    `
  });
  ...
  const closeBtn = findElement('#modal-close');
  const cancelBtn = findElement('#cancel-game');
  const singlePlayerBtn = findElement('#single-player-btn');
  const multiplayerBtn = findElement('#multiplayer-btn');
  const tournamentBtn = findElement('#ai-tournament-btn');
  ...
  if (singlePlayerBtn) {
    singlePlayerBtn.addEventListener('click', () => {
      ...
      this.closeModal();
      this.showToast('success', 'Game Starting!', 'Single player mode selected');
  ...
  if (multiplayerBtn) {
    multiplayerBtn.addEventListener('click', () => {
      ...
      this.closeModal();
      this.showToast('success', 'Game Starting!', 'Multiplayer mode selected');
  ...
  if (tournamentBtn) {
    tournamentBtn.addEventListener('click', () => {
      console.log('🏆 Tournament selected');
      this.closeModal();
      this.showToast('success', 'Game Starting!', 'Tournament mode selected');
